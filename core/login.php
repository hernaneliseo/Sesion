<?php
	...
	if ($_SERVER["REQUEST_METHOD"] == "POST") {
		$usuario=check_input($_POST['username']);
		...
		*/
		$query=mysqli_query("SELECT * FROM usuarios WHERE idUsuario='$usuario'");
		...
		if(mysqli_num_rows($query)==0){
			...
			if ($row['nivel_usu']==1){
				$_SESSION['codigousuario']=$row['idusuario'];
				?>
				<script>
